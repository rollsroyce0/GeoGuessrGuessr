...
predicted_coords_pic1 = []
predicted_coords_pic2 = []
predicted_coords_pic3 = []
predicted_coords_pic4 = []
predicted_coords_pic5 = []
...
if __name__ == "__main__":
    ...
    for name in os.listdir('Roy/ML/Saved_Models'):
        ...
        for image_path in os.listdir('Roy/Test_Images'):
            ...
            if counter == 0:
                predicted_coords_pic1.append(predicted_coords)
            elif counter == 1:
                predicted_coords_pic2.append(predicted_coords)
            elif counter == 2:
                predicted_coords_pic3.append(predicted_coords)
            elif counter == 3:  
                predicted_coords_pic4.append(predicted_coords)
            elif counter == 4:
                predicted_coords_pic5.append(predicted_coords)
        ...
        if total < 14000:
            # remove the model from the combined list
            predicted_coords_pic1 = predicted_coords_pic1[:-1]
            predicted_coords_pic2 = predicted_coords_pic2[:-1]
            predicted_coords_pic3 = predicted_coords_pic3[:-1]
            predicted_coords_pic4 = predicted_coords_pic4[:-1]
            predicted_coords_pic5 = predicted_coords_pic5[:-1]
        
        print(f"Model {name} finished with {total} points")
        
    list_of_predicted_coords = [predicted_coords_pic1, predicted_coords_pic2, predicted_coords_pic3, predicted_coords_pic4, predicted_coords_pic5]    
    
    
    
    # for each image, average the predicted coordinates
    predicted_coords_pic1 = np.mean(predicted_coords_pic1, axis=0)
    predicted_coords_pic2 = np.mean(predicted_coords_pic2, axis=0)
    predicted_coords_pic3 = np.mean(predicted_coords_pic3, axis=0)
    predicted_coords_pic4 = np.mean(predicted_coords_pic4, axis=0)
    predicted_coords_pic5 = np.mean(predicted_coords_pic5, axis=0)
    print(predicted_coords_pic1)
    
    # Calculate the errors for each image
    errors = []
    errors.append(haversine(real_coordinates[0], predicted_coords_pic1))
    errors.append(haversine(real_coordinates[1], predicted_coords_pic2))
    errors.append(haversine(real_coordinates[2], predicted_coords_pic3))
    errors.append(haversine(real_coordinates[3], predicted_coords_pic4))
    errors.append(haversine(real_coordinates[4], predicted_coords_pic5))
    print(errors)
        
    print("Errors for each image:")
    for i, error in enumerate(errors):
        print(f"Image {i+1}: {error} km")
        
    # point calculation for each image
    points = []
    for error in errors:
        points.append(geoguessr_points_formula(error))
        
    print("Points for each image:")
    for i, point in enumerate(points):
        print(f"Image {i+1}: {point} points")
    total = 0
    for point in points:
        total += point
    print(f"Total Points for combined model: {total}")
    
    # Please visualize the predicted coordinates on the world map
    plot_coordinates_on_map(predicted_coords_pic1, "Roy/Test_Images/Game_1.jpg")
    plot_coordinates_on_map(predicted_coords_pic2, "Roy/Test_Images/Game_2.jpg")
    plot_coordinates_on_map(predicted_coords_pic3, "Roy/Test_Images/Game_3.jpg")
    plot_coordinates_on_map(predicted_coords_pic4, "Roy/Test_Images/Game_4.jpg")
    plot_coordinates_on_map(predicted_coords_pic5, "Roy/Test_Images/Game_5.jpg")
