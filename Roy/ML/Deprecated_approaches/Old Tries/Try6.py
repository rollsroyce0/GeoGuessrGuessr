...
print("Training DecisionTreeClassifier on continent labels...")
dt_classifier = RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1)
...
for continent in track(np.unique(train_labels), description="Processing continents..."):
    ...
    for sub_label in range(10):
        sub_cluster_data = continent_data[sub_labels == sub_label]
        sub_cluster_coordinates = continent_coordinates[sub_labels == sub_label]

        dt_regressor = Decision()
        dt_regressor.fit(sub_cluster_data, sub_cluster_coordinates)

        # Store the model for later prediction
        sub_cluster_models[(continent, sub_label)] = dt_regressor
...
mse = evaluate_model(X_test, y_test, dt_classifier, sub_cluster_models, sub_clusters)
