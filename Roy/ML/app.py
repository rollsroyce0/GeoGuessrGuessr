...
from templates import Geoguessrmodel_Evaluator_Multimodel_silent as regular
from templates import Geoguessrmodel_Evaluator_Multimodel_silent_check as check

app = Flask(__name__)
list_of_maps = ['All', 'Best', 'Game', 'Validation', 'Super', 'Verification', 'Ultra', 'Extreme', 'Chrome',
...
def index():
    if request.method == 'POST':
        ...
        include_checkpoints = 'include_checkpoints' in request.form
        print(f"Selected test type: {testtype}, Include checkpoints: {include_checkpoints}")
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            #return "Including checkpoints is currently not supported in this interface."
            if testtype == 'All':
                final_temp, highest_temp, difficulty_temp, avg_scores_temp, median_scores_temp, pred_coords_temp, real_coords_temp, errors_temp, points_temp, img_paths_temp = [], [], [], [], [], [], [], [], [], []
                final, highest, difficulty, avg_scores, median_scores, pred_coords, real_coords, errors, points, img_paths = [], [], [], [], [], [], [], [], [], []
                for map_type in list_of_maps:
