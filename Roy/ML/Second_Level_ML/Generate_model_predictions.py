...
def main2(testtype=None, predicted_coordinates=None):
    ...
    # get the real coordinates using the new function
    else:
    ...
    difficulty_scores = np.std(errors, axis=0)
    return sum(final_pts), sum(highest_points), np.round(np.mean(difficulty_scores), 3), avg_scores, median_scores, avg_preds, real_coords, final_errs, final_pts, img_paths, highest_points, predicted_coordinates
...
def main():
    ...
    predicted_coordinates = pd.DataFrame(columns=['test_type','Model_Name',  'Predicted_Latitude1', 'Predicted_Longitude1',
                                                 ...
                                                 'Predicted_Latitude5', 'Predicted_Longitude5'])
    
    if testtype == 'All':
        for testtype in list_of_maps:
            print("\n----------------------------------------------------------------------\n")
            #print(f"Running test for {testtype}...")
            final_score, highest_score, difficulty_score, avg_scores, median_scores, avg_preds, real_coords, final_errs, final_pts, img_paths, highest_points, predicted_coordinates = main2(testtype, predicted_coordinates)
        ...
        predicted_coordinates.to_csv('Roy/ML/Second_Level_ML/predicted_coordinates_all_testtypes.csv', index=False)
        ...
        predicted_coordinates.to_csv('Roy/ML/Second_Level_ML/predicted_coordinates_'+testtype+'.csv', index=False)
