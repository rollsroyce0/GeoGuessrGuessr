...
class ResGeo(nn.Module):
    def __init__(self):
        super().__init__()
        # after two means we have a single scalar → so Linear(1, 1)
        self.linear1 = nn.Linear(3,2000)
        self.linear2 = nn.Linear(2000, 2)  # Output layer for latitude and longitude

    def forward(self, x):
        # 1) collapse everything to a single scalar
        x = x.mean(-1).mean(-1)  # → [B, C] where C is the number of channels
        # 2) make it a 1D tensor
              # → [1, 1]
        
        # 3) apply linear
        x = self.linear1(x) 

        # 5) apply tanh to get the output in the range [-1, 1]
        x = torch.tanh(x)
        x = self.linear2(x)  # → [B, 2] where 2 is for latitude and longitude
        # 6) ensure the output is in the range [-90, 90] for latitude and [-180, 180] for longitude
        x[:, 0] = (x[:, 0] + 90) % 180 - 90
        x[:, 1] = (x[:, 1] + 180) % 360 - 180      
        #print(x)
        return x
...
geo_predictor = ResGeo().to(device)
...
criterion = MSELoss()
optimizer = optim.AdamW(geo_predictor.parameters(), lr=1e-2, weight_decay=4e-4, amsgrad=True)
