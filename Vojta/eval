...
if __name__ == "__main__":
    ...
    # Load the models
    geo_embedding_model = torch.compile(GeoEmbeddingModel().to(device))
    geo_predictor_nn = torch.compile(GeoPredictorNN().to(device))

    # Load the saved model weights
    #geo_embedding_model.load_state_dict(torch.load('./Saved_Models/geo_embedding_model_r152_normal.pth', map_location=device))
    geo_predictor_nn.load_state_dict(torch.load('./Saved_Models/low_geo_predictor_nn_lowest.pth', map_location=device))
    ...
    for image_path in os.listdir('./Test_Images'):
        ...
        image_path = f"./Test_Images/{image_path}"
