...
#######################################
embeddings_file = '/home/TheSmilingTurtle/Downloads/Roy_Files/Embeddings/embeddings.npy'
image_paths_file = '/home/TheSmilingTurtle/Downloads/Roy_Files/Embeddings/image_paths.npy'
...
if os.path.exists(embeddings_file):
    ...
    if os.path.exists('./Saved_Models/geo_embedding_model.pth'):
        model.load_state_dict(torch.load('./Saved_Models/geo_embedding_model.pth'))
    ...
    torch.save(model.state_dict(), './Saved_Models/geo_embedding_model.pth')
...
geo_predictor = torch.compile(GeoPredictorNN().to(device))
...
criterion = torch.compile(haversine_loss)
optimizer = optim.AdamW(geo_predictor.parameters(), lr=1e-4, weight_decay=1e-3)
...
# Training Loop                         #
#######################################
train_loader = DataLoader(list(zip(X_train, y_train)), batch_size=2048, shuffle=True)
epochs = 500
losses = []
val_losses = []
min_val_loss = 1e10
...
for epoch in track(range(epochs), description="Training the model..."):
    ...
    for embeddings_batch, coords_batch in train_loader:
        ...
        coords_batch = coords_batch.float()
        optimizer.zero_grad()
    ...
    with torch.no_grad():
        ...
        val_loss = haversine_loss(geo_predictor(X_test_tensor), y_test_tensor)
    ...
    if val_loss.item() < min_val_loss:
        ...
        torch.save(geo_predictor.state_dict(), './Saved_Models/low_geo_predictor_nn_lowest.pth')
    else:
        counter += 1
    ...
    if (epoch + 1) % 10 == 0:
        print(f"Running average: {np.mean(val_losses[-10:]):.4f} with learning rate: {optimizer.param_groups[0]['lr']} with counter: {counter}")

    if (epoch + 1) % 3 == 0 and val_loss.item() < 1.1 * np.min(val_losses) and val_loss.item() < 1000:
        torch.save(geo_predictor.state_dict(), f'./Saved_Models/Checkpoint_Models_NN/geo_predictor_nn_{epoch}_loss_{np.round(val_loss.item(), 0)}.pth')

print('Finished Training')
torch.save(geo_predictor.state_dict(), './Saved_Models/geo_predictor_nn.pth')
...
def evaluate_nn_model(X_test, y_test, geo_predictor):
    ...
    distances = np.array([haversine_distance(y_test.cpu()[i], y_pred[i]) for i in range(len(y_test))])
...
geo_predictor.load_state_dict(torch.load('./Saved_Models/geo_predictor_nn.pth'))
...
plt.scatter(y_test.cpu()[:100, 1], y_test.cpu()[:100, 0], color='blue', label='True')
...
for i in range(100):
    plt.plot([y_test.cpu()[i, 1], y_pred[i, 1]], [y_test.cpu()[i, 0], y_pred[i, 0]], color='green', alpha=0.5)
...
world = gpd.read_file(url)
world.boundary.plot(ax=plt.gca(), linewidth=1, color='black')
plt.scatter(true_coords_smallest.cpu()[:, 1], true_coords_smallest.cpu()[:, 0], color='blue', label='True')
plt.scatter(predicted_coords_smallest[:, 1], predicted_coords_smallest[:, 0], color='red', label='Predicted')
for i in range(100):
    plt.plot([true_coords_smallest.cpu()[i, 1], predicted_coords_smallest[i, 1]],
             [true_coords_smallest.cpu()[i, 0], predicted_coords_smallest[i, 0]],
